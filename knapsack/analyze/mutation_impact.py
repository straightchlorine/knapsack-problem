...
def mutation_impact(
    ...
    results = _measure_mutation_impact(algorithm, config, iterations)

    plot_mutation_performance(results)
...
def plot_mutation_performance(results: dict[str, ExperimentResults]):
    """
    Plot the comparison of mutation rates for fitness metrics (best, average, worst)
    ...
    colors = plt.cm.get_cmap("tab10", len(mutation_rates))
    ...
    for i, (metric, title) in enumerate(zip(metrics, titles)):
        for j, operator in enumerate(mutation_rates):
            # Get data
...
def plot_diversity(results: dict[str, ExperimentResults]):
    ...
    for i, operator in enumerate(mutation_rates):
...
def plot_execution_times(results: dict[str, ExperimentResults]):
    ...
    execution_times = [
        results[operator].mean_execution_time for operator in mutation_rates
    ]

    bars = plt.bar(mutation_rates, execution_times)
...
def plot_optimal_generations(results: dict[str, ExperimentResults]):
    ...
    optimal_gens = [
        results[operator].mean_optimal_generation for operator in mutation_rates
    ]

    # Create bars with different colors
    bars = plt.bar(mutation_rates, optimal_gens)
